<!-- =======================
     ANIMATED TYPING HEADER
======================= -->

<p align="center">
  <img 
    src="https://readme-typing-svg.herokuapp.com?font=Fira+Code&size=26&pause=1200&color=CF9FFF&center=true&vCenter=true&width=650&lines=Hi%2C+I'm+Om+Joshi;AI+%26+Full+Stack+Developer;B.E.+IT+Student+%7C+Pune;Building+Real+World+Tech+Solutions&cacheBust=1" 
    alt="Typing SVG"
  />
</p>



---

## 👋 About Me

Hey! I’m **Om Joshi**, a **B.E. Information Technology student**  
🎓 *Nutan Maharashtra Institute of Engineering and Technology, Pune*

I’m passionate about building **real-world, useful software** — not just tutorial projects.  
I enjoy turning ideas into **clean, scalable, and production-ready solutions**.

### What I love working on:
- 🌐 **Full-Stack Web Development**
- 🤖 **Artificial Intelligence & Automation**
- 🧠 **Problem Solving & Clean Code**

---

## 🎯 Career Focus

- 💼 Actively seeking **Internships** (Web / Full-Stack / AI)
- 🚀 Strong fundamentals in **frontend + backend development**
- 🤖 Exploring **AI APIs, automation, and ML basics**
- 📈 Growth mindset with **project-driven learning**

---

## 🧠 Tech Stack

### 👨‍💻 Programming & Core
<p>
  <img src="https://skillicons.dev/icons?i=c,cpp,python,javascript" />
</p>

### 🌐 Web Development
<p>
  <img src="https://skillicons.dev/icons?i=html,css,javascript" />
</p>

### 🤖 Tools, Cloud & DevOps
<p>
  <img src="https://skillicons.dev/icons?i=git,github,gcp" />
</p>

---

## 🤖 AI & Automation Focus

I’m actively building skills in **applied AI**, including:

- 🔹 Integrating **AI APIs** (OpenAI / Gemini – integration ready)
- 🔹 Automating workflows using **Python**
- 🔹 Prompt engineering & AI-assisted development
- 🔹 Learning ML fundamentals (data → model → output)

> 🎯 **Goal:** Build *AI-powered tools* that solve real problems — not just demos.

---

## 🧠 Featured Projects

### 🔹 Personal Portfolio Website
**Tech:** HTML, CSS, JavaScript  
- Fully responsive & modern UI  
- Smooth animations and interactions  
- Clean, structured codebase  

➡️ *Live demo & source code coming soon*

---

### 🔹 AI Chat Assistant *(In Progress)*
**Tech:** Python, FastAPI, AI APIs  
- Conversational AI backend  
- Modular & scalable architecture  
- API-first design  

➡️ *Internship-grade AI project*

---

### 🔹 Automation Scripts
**Tech:** Python  
- Task automation & productivity tools  
- File handling & data processing  
- Reusable, clean scripts  

➡️ *Focused on real-world usefulness*

---

> 🧩 *More quality projects coming soon — quality over quantity.*

---

## 📊 GitHub Stats

<p align="center">
  <img 
    src="https://github-readme-stats.vercel.app/api?username=omjoshi-2307&show_icons=true&theme=tokyonight&hide_border=true" 
  />
</p>

<p align="center">
  <img 
    src="https://github-readme-streak-stats.herokuapp.com/?user=omjoshi-2307&theme=tokyonight&hide_border=true" 
  />
</p>

<p align="center">
  <img 
    src="https://github-readme-stats.vercel.app/api/top-langs/?username=omjoshi-2307&layout=compact&theme=tokyonight&hide_border=true" 
  />
</p>

---

## 🌐 Connect With Me

<p align="center">
  <a href="https://www.linkedin.com/in/0m-joshi2307">
    <img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
<p align="center">
  <img src="typing-header.svg" alt="Typing Header" />
</p>


  <a href="mailto:omjoshi2307@gmail.com">
    <img src="https://img.shields.io/badge/Email-D14836?style=for-the-badge&logo=gmail&logoColor=white"/>
  </a>
</p>

---

## 📌 Final Note

✨ I’m not just learning technology —  
I’m preparing to **create value in real engineering teams**.

If you’re a recruiter, mentor, or fellow developer,  
**let’s connect and build something meaningful together.** 🚀

---

<p align="center">
  <img src="https://img.shields.io/badge/Visitors-1000-00F7FF?style=for-the-badge" alt="Visitor Count" />
</p>
